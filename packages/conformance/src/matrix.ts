...
export const CONFORMANCE_MATRIX: ConformanceEntry[] = [
  ...
  // Handoff 17 ES-1 (§4a): the event-subprocess attributes as standard OMG.
  { element: 'bpmn:subProcess @triggeredByEvent', status: 'supported', conformanceClass: 'analytic', mappedTo: 'properties.triggeredByEvent', notes: 'Event subprocess (F7 containment reused whole); sequence flow to/from the shell is vetoed by the default rules; exempt from UNREACHABLE_NODE.' },
  { element: 'bpmn:startEvent @isInterrupting', status: 'supported', conformanceClass: 'analytic', mappedTo: 'properties.isInterrupting', notes: 'OMG default (true) omitted on export; only isInterrupting="false" is written. Round-trips wherever it appears — semantics are policed by lint, never the converter.' },
