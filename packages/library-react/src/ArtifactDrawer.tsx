 ...
 */
export function ArtifactDrawer({ detail, onAction, onClose }: ArtifactDrawerProps) {
  ...
  return (
    <aside className="btv-lib-drawer" aria-label={`Detalhe do artefato ${detail.name}`}>
      <div className="btv-lib-drawer-head">
        <span className="btv-lib-kicker">DETALHE · {detail.typeLabel}</span>
        <button type="button" className="btv-lib-drawer-close" onClick={onClose} aria-label="Fechar detalhe">
          ✕
        </button>
      </div>
        ...
        <section className="btv-lib-box">
          {detail.effectiveFrom && (
            <p className="btv-lib-box-line">
              <strong>Vigência:</strong> desde {formatDate(detail.effectiveFrom)}
              {detail.effectiveUntil ? ` até ${formatDate(detail.effectiveUntil)}` : ''}
            </p>
          )}
          {!detail.effectiveFrom && detail.effectiveUntil && (
            <p className="btv-lib-box-line">
              <strong>Vigência:</strong> até {formatDate(detail.effectiveUntil)}
            </p>
          )}
          {(detail.approvers?.length ?? 0) > 0 && (
            <p className="btv-lib-box-line">
              <strong>Aprovação:</strong> {detail.approvers!.join(', ')}
            </p>
          )}
      ...
      {detail.provenance && (
        <section className="btv-lib-box btv-lib-provenance">
          <span className="btv-lib-kicker">PROVENIÊNCIA</span>
      ...
      {detail.versions.length > 0 && (
        <section className="btv-lib-box">
          <span className="btv-lib-kicker">VERSÕES</span>
