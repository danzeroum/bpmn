...
## PARTE A — `@bpmn-react/conformance` (F8 como produto)

Novo pacote `packages/conformance`. Headless, zero deps, consome só `@bpmn-react/core`.
...
## PARTE C — `@bpmn-react/soundness` (análise semântica de processos)
...
## PARTE B — `@bpmn-react/audit` (integridade demonstrável + XES)

Novo pacote `packages/audit`. Headless, zero deps, consome `@bpmn-react/core` + `@bpmn-react/registry`.
