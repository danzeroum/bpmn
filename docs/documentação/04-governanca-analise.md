...
`conformance`, `soundness`, `replay` e `lint`. Para cada arquivo `src/` documentam-se **todos** os dados
que circulam — entradas, dados intermediários (mesmo quando se perdem na própria classe/função) e saídas.
...
conformidade, `RunBinding` congelado, `VerificationReport`/`ReplayAnalysis` em JSON, achados de lint
(`LintFinding`/`ValidationIssue`) com quick-fixes (`Command`) e verdicts de promoção. Nenhum pacote faz
rede: transportes (git/TSA/S3), chaves e relógio são **injetados** pelo host.
...
> Δ 2026-07: errata do H17 (#135) — `eventSubProcess` marcado como `supported` (analytic, mapeado a `subProcess (triggeredByEvent)`). Novas linhas `supported` na matriz OMG: `bpmn:subProcess @triggeredByEvent`, `bpmn:startEvent @isInterrupting`, os quatro roots nomeados (`bpmn:message/signal/error (root)` + `bpmn:escalation (root)` — H18 §5a), `bpmn:escalationEventDefinition` e `bpmn:compensateEventDefinition` (H19 §6a, completa a família de gatilhos OMG), `bpmn:timeDate/timeDuration/timeCycle` (H16 E-5), e `bpmn:complexGateway` (extended). A matriz agora tem 56 linhas.
...
**Saídas:** `CONFORMANCE_MATRIX: ConformanceEntry[]` (56 linhas: `element`, `status`, `conformanceClass`, `mappedTo?`, `notes?`); número de cobertura por classe. Elementos deliberadamente `unsupported`: `bpmn:transaction`, `bpmn:adHocSubProcess`, `conversation / choreography diagrams`.
...
> Δ 2026-07: `LOCAL_TO_MATRIX` ganhou os elementos novos do perfil — root `escalation` (H18 §5a), `escalationEventDefinition`/`compensateEventDefinition` e `complexGateway` — de modo que documentos que os usam classificam (não caem em `unsupportedElements`). Estrutura de `CertifyReport` inalterada.
...
> Δ 2026-07: `GENERATED_CORPUS_FILES` subiu de 58 para 60 (novos equivalentes estruturais commitados, ainda anti-drift-testados contra o diretório).
...
**Saídas:** `GENERATED_CORPUS_FILES = 60`; `EXTERNAL_CORPUS_MIN = 20`; `EXTERNAL_CORPUS_MAX = 40`; `EXTERNAL_CORPUS_SOURCES` (bpmn-io/bpmn-js-examples MIT, camunda/camunda-get-started-quickstart Apache-2.0).
**Estruturas de dados que trafegam:** Constantes numéricas e a lista de fontes com licença.

### `packages/conformance/src/render.ts`
> Δ 2026-07: além da tabela da matriz, agora renderiza a tabela comparativa de terceiros a partir de `THIRD_PARTY_DECLARATIONS` (Handoff 14 §1g, com a regra de honestidade em bloco de citação) e a seção de passthrough de extensões estrangeiras (`zeebe:*`/`camunda:*`).
**Papel:** Renderiza `CONFORMANCE.md` determinística e reprodutivelmente a partir da matriz (freshness-checked em CI).
**Entradas:** `renderConformanceMarkdown(entries = CONFORMANCE_MATRIX)`. Usa `classCoverage`, as constantes de `corpusPolicy` e `THIRD_PARTY_DECLARATIONS` de `thirdParty`.
**Processamento (intermediário):** Mapa `STATUS_BADGE` (emoji por status); acumula `lines: string[]` — cabeçalho gerado, percentuais descriptive/analytic, tabela Markdown por entrada; tabela comparativa "declarações de terceiros" (célula = `d.claim` quando `d.declaredElements.includes(entry.element)`, senão `—`, com link por coluna e nota de honestidade); seção de passthrough de extensões estrangeiras; seção corpus real vs gerado (contagens de `corpusPolicy`), seção `certify --strict` vs XSD e exit codes.
**Saídas:** String Markdown (documento `CONFORMANCE.md`) — sai para o repositório.
**Estruturas de dados que trafegam:** `ConformanceEntry`, `ThirdPartyDeclaration`, constantes de corpus.

### `packages/conformance/src/thirdParty.ts`
> Δ 2026-07: arquivo novo (Handoff 14 §1g) — colunas comparativas de terceiros para o `CONFORMANCE.md`.
**Papel:** Fixture das declarações comparativas de terceiros. Regra de honestidade **vinculante**: cada célula reflete SOMENTE o que a documentação do próprio fornecedor declara (link por coluna) — nunca teste ou claim nosso sobre concorrente. Atualizar uma coluna = reler a doc DELES e ajustar este fixture.
**Entradas:** Nenhuma em runtime (constrói `SPEC_ELEMENTS` de `CONFORMANCE_MATRIX`, excluindo a extensão própria `btv:*`).
**Processamento (intermediário):** `SPEC_ELEMENTS` = elementos da matriz sem prefixo `btv:`; a coluna `bpmn-js (bpmn.io)` declara todos os `SPEC_ELEMENTS` menos `conversation / choreography diagrams`; a coluna `Camunda 8 (Zeebe)` lista explicitamente os elementos que a página de coverage deles marca como implementados (ausências conspícuas — complexGateway, conditional events, transaction/adHoc — ficam fora, renderizando `—`).
**Saídas:** `THIRD_PARTY_DECLARATIONS: ThirdPartyDeclaration[]` (por fornecedor: `vendor`, `sourceUrl`, `claim`, `declaredElements[]`). Um elemento ausente de `declaredElements` vira `—` (sem declaração registrada), que **não** é claim de ausência.
**Estruturas de dados que trafegam:** `ThirdPartyDeclaration`, `CONFORMANCE_MATRIX`.

### `packages/conformance/src/index.ts`
> Δ 2026-07: passa a re-exportar `thirdParty` (colunas comparativas).
**Papel:** Barrel público do conformance.
**Entradas:** Re-exporta matrix, render, thirdParty, manifest, certify, corpusPolicy.
**Processamento (intermediário):** Nenhum.
**Saídas:** Superfície pública.
...
**Entradas:** `buildScopeGraphs(diagram: BpmnDiagram)`; `reachableFrom(graph, seeds)`; `coReachableTo(graph, seeds)`; `cyclicComponents(graph)`; helpers `isFlowNode`/`isFlowEdge`/`flowScopeOf` (reexportados de core). De core: `activeNodes`, `activeEdges`, `boundaryAttachedTo`, `flowScopeOf`, `isFlowNode`, `isFlowEdge`, `BpmnDiagram`, `BpmnNode`.
**Processamento (intermediário):** A classificação de flow node/edge é delegada a `isFlowNode`/`isFlowEdge` de core (`model/flow.ts`, que já sabe excluir dataObject/dataStore/textAnnotation/group e messageFlow/association/dataAssociation); exclui também elementos fechados via `activeNodes`/`activeEdges`. O escopo por nó vem de `flowScopeOf` (que resolve corretamente subprocessos e event subprocesses). `Map` `graphs` por escopo e `scopeOf` por nó; para cada `ScopeGraph` monta `nodes`/`out`/`in`/`starts`; liga arestas apenas quando ambos endpoints existem no MESMO escopo; adiciona arestas sintéticas `implicit` host→boundaryEvent (só para alcançabilidade); `ends` = endEvents ou nós-sink (sem saída); BFS forward/backward com `Set`+fila; Tarjan **iterativo** (frames com cursor de aresta, `indices`/`lowlinks`/`onStack`/`stack`) retornando só componentes cíclicos reais (≥2 nós ou self-loop).
...
> Δ 2026-07: pacote novo `@buildtovalue/lint` (package v1.1.0). Lint de modelagem estilo bpmnlint (referência item 7) — regras "vivas" que rodam SOBRE a validação estrutural (core) e a análise de soundness, entregues como `ValidationRule`s compatíveis com o sistema de plugins. Dois perfis versionados coexistem na MESMA superfície de painel: **Etiqueta** (estilo: coisas que parseiam e até rodam, mas dificultam ler/revisar) e **Prontidão de execução** (engine-readiness: coisas que um motor Camunda/Zeebe rejeita ou interpreta mal). Todo achado carrega um código `UPPER_SNAKE` estável para o host allowlistar regras, mapear severidades ou anexar quick-fixes. **Nunca faz rede nem toca em chaves**; opera puro sobre `BpmnDiagram` de core. Um único arquivo-fonte (`src/index.ts`).

### `packages/lint/src/index.ts`
> Δ 2026-07: arquivo novo. Catálogo de regras cresceu por handoff (E-5 §3d → H17 ES-4 §4d → H18 §5d → H19 §6c), com os perfis promovendo a versão a cada onda.
**Papel:** Define o catálogo de regras vivas de lint estrutural, os dois perfis versionados, os quick-fixes mecânicos e as funções que dobram tudo em achados/resultado — a fronteira "diagrama → issues de lint (+ comandos de correção)".
**Entradas:** `lintDiagram(diagram: BpmnDiagram, rules = ALL_LINT_RULES)`; `lintFindings(diagram, profiles = LINT_PROFILES)`; `fixCommandFor(diagram, finding: LintFinding, profiles = LINT_PROFILES)`; cada `ValidationRule` exportada recebe um `BpmnDiagram`; os dois builders compartilhados `typedMessageStartCommands(diagram, {parentId?,x,y,definitionName?})` e `compensationHandlerCommands(diagram, {boundary,host,handlerName?})`. De core: leitores/classificadores `activeNodes`, `activeEdges`, `isFlowNode`, `isFlowEdge`, `isEventType`, `isEventSubprocess`, `isContainerType`, `childrenOf`, `nodeParentId`, `boundaryAttachedTo`, `flowScopeOf`, `eligibleEscalationCatches`, `eventDefinitionRefOf`, `compensableActivitiesOf`, `timerPropertyOf`, `parseTimerExpression`, `nextEventDefinitionId`; construtores de comando `createNode`, `createEdge`, `addNodeCommand`, `addEdgeCommand`, `addEventDefinitionCommand`, `updateNodeCommand`, `removeNodeCommand`, `removeEdgeCommand`, `compositeCommand`; tipos `BpmnDiagram`, `BpmnNode`, `BpmnEdge`, `Command`, `ValidationIssue`, `ValidationResult`, `ValidationRule`.
**Processamento (intermediário):** Heurísticas registry-free `isActivity` (contém "task" | subProcess | callActivity), `isGateway` (contém "gateway"), `flowCounts` (mapas incoming/outgoing só de `isFlowEdge`), `flowEdgesTouching`; leitura de "kind" via `eventKindOf(node)` = `node.properties.eventDefinition` (string). Conjuntos-chave de kind (fonte única compartilhada): `START_FORBIDDEN_KINDS` = {terminate, link}; `END_FORBIDDEN_KINDS` = {timer, conditional, link}; `NAMED_REF_KINDS` = {message, signal, error, escalation}; `SUBPROC_TRIGGER_KINDS` = [message, signal, error, timer, conditional, escalation, compensate] (H18 §5d adiciona escalation, H19 §6c adiciona compensate). Catálogo de regras — **Etiqueta**: `LINT_LABEL_REQUIRED` (warning — todo flow node/container exceto gateway/annotation precisa de label), `LINT_SUPERFLUOUS_GATEWAY` (warning — gateway 1-in/1-out não faz nada; quick-fix), `LINT_IMPLICIT_SPLIT`/`LINT_IMPLICIT_JOIN` (warning — atividade com 2+ saídas/entradas esconde decisão/merge), `LINT_DUPLICATE_FLOW` (warning — duas arestas mesmo src→tgt; quick-fix), `LINT_START_WITH_INCOMING`/`LINT_END_WITH_OUTGOING` (error — endpoints de evento; quick-fix); regras de evento `EVT_START_THROW`/`EVT_END_CATCH` (error — start só captura / end só lança), `EVT_ERROR_START_TOPLEVEL` (error — error start fora de event subprocess), `EVT_ESCALATION_START_TOPLEVEL` (error — escalation start fora de event subprocess, H18 §5d), `EVT_ESCALATION_CATCH_ILLEGAL` (error — escalation num intermediateCatchEvent), `ESC_NO_CATCH` (warning — throw de escalação sem catch elegível via `eligibleEscalationCatches`; escalação sem destino DISSOLVE, legal na OMG), `EVT_SUBPROC_FLOW` (error — sequence flow toca a casca do event subprocess, um achado por aresta), `EVT_SUBPROC_START` (error — event subprocess precisa de EXATAMENTE 1 start tipado entre filhos diretos; três falhas: 0 / >1 / kind faltando ou fora de `SUBPROC_TRIGGER_KINDS`; quick-fix só no caso 0-starts); regras de compensação (H19 §6c) `COMP_HANDLER_FLOW` (error — sequence flow toca um handler `isForCompensation`), `COMP_BOUNDARY_NO_HANDLER` (error — boundary de compensação sem association; quick-fix), `COMP_REF_NOT_COMPENSABLE` (warning — throw cujo `compensateActivityRef` não tem boundary de compensação no escopo via `compensableActivitiesOf`+`flowScopeOf`), `COMP_CATCH_ATTRS` (warning — catch de compensação carregando activityRef/waitForCompletion que a OMG reserva ao throw), `COMP_START_TOPLEVEL` (error — compensation start fora de event subprocess). **Prontidão de execução**: `EXEC_MISSING_IMPLEMENTATION` (warning — service/send/businessRuleTask sem binding entre `implementation`/`taskDefinitionType`/`zeebe:*`/`camunda:*`/`decisionRef`), `EXEC_UNCONDITIONED_FLOWS` (warning — exclusive/inclusive fork com 2+ fluxos sem condição nem default), `EVT_REF_MISSING` (warning — evento de kind nomeado sem `eventDefinitionRef`; quick-fix cria e referencia a definição), `TIMER_MALFORMED` (error — expressão de timer presente mas inválida, decidido pelo `parseTimerExpression`). Quick-fixes retornam UM `Command` undoable (composites dobram multi-passo em um só undo) ou `null` quando não há correção mecânica: `fixDuplicateFlow`, `fixSuperfluousGateway`, `fixEventEndpoints`, `fixEvtRefMissing`, `fixEvtSubprocStart` (via builder compartilhado `typedMessageStartCommands`), `fixCompBoundaryNoHandler` (via `compensationHandlerCommands`). `lintDiagram` faz `rules.flatMap(rule => rule(diagram))` e deriva `valid` (nenhum error). `lintFindings` percorre os perfis anotando cada issue com `ruleId`/`profileId`/`source` e `fixable` (chama `rule.fix` contra o diagrama corrente). `fixCommandFor` reconstrói um comando FRESCO para um finding (comandos fecham sobre estado de undo no execute — nunca reusar entre execuções).
**Saídas:** Por regra, `ValidationIssue[]` (`code`, `severity` `error|warning`, `message` em EN, `nodeId?`/`edgeId?`); `lintDiagram` → `ValidationResult` (`valid`, `issues`); `lintFindings` → `LintFinding[]` (issue + `ruleId`, `profileId`, `source`, `fixable`); `fixCommandFor`/fixes → `Command | null`; builders compartilhados → `{commands: Command[], …ids}`. Arrays exportados `ETIQUETTE_RULES`, `EXECUTABILITY_RULES`, `ALL_LINT_RULES`; perfis `ETIQUETTE_PROFILE` (`id:'lint-etiquette'`, name "Etiqueta de modelagem", **version 1.4.0**, source `etiquette`, 19 regras), `EXECUTABILITY_PROFILE` (`id:'lint-engine'`, name "Prontidão de execução (engine)", **version 1.4.0**, source `executability`, 4 regras), `LINT_PROFILES`. Versões de perfil evoluíram por handoff: 1.0.0 → 1.1.0 (E-5 §3d) → 1.2.0 (H17 ES-4 §4d) → 1.3.0 (H18 §5d) → 1.4.0 (H19 §6c) — cada onda de regras = nova versão promovível que o cabeçalho do painel e o adapter da Biblioteca refletem desta fonte única.
**Estruturas de dados que trafegam:** `LintRule` (`id` kebab-case, `run: ValidationRule`, `fix?: (LintFixContext) => Command | null`), `LintProfile` (`id`, `name`, `version`, `source: 'etiquette'|'executability'`, `rules: LintRule[]`), `LintFinding` (estende `ValidationIssue` com `ruleId`/`profileId`/`source`/`fixable`), `LintFixContext` (`diagram`, `issue`), `ValidationIssue`, `ValidationResult`, `ValidationRule`, `Command`, `BpmnDiagram`/`BpmnNode`/`BpmnEdge`.
