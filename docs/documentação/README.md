...
| Tipo | Qtd. | Situação |
|---|---|---|
| `.ts` / `.tsx` (TypeScript) | ~600 (≈309 em `src/`) | **100% da lógica** — todo o dado do sistema trafega aqui |
| `.html` | 1 | `packages/example/index.html` (bootstrap do app-demo Vite) |
| `.css` | 4 | folhas de estilo dos pacotes React (tokens/temas) |
| `.bpmn` (XML) | 65+ | `packages/conformance/corpus/` — **dados de entrada** de teste de interop |
...
| [`04-governanca-analise.md`](04-governanca-analise.md) | Governança · Confiança · Análise | `registry`, `audit`, `identity`, `anchor-*`, `conformance`, `soundness`, `replay`, `lint` |
...
**Cobertura: 100% dos `src/` dos 25 pacotes** (≈309 arquivos hoje — contagens absolutas
...
| `BpmnVersion` | `{id,semanticVersion,status,approvedBy,changeSummary,snapshotHash,parentVersionId}` (status ∈ 7 estados: draft/**in-review**/test/candidate/active/deprecated/retired) | lifecycle → registry → audit |
...
| `EventDefinition` (nomeada) | `{id,kind,name?,...}` — root element OMG (message/timer/error/signal/escalation/compensate/…) | catálogo `diagram.definitions` → binding em nós/timers → persistência/lint |
| `XmlSubtree` (`foreign`) | subárvore de extensões estrangeiras (`zeebe:*`/`camunda:*`) preservada verbatim | round-trip lossless em nós/arestas/diagrama |
...
| `LintFinding` / `LintProfile` | `{code,severity,message,fix?}` + perfil versionado (`lint-etiquette`/`lint-engine`, semver por handoff) | lint estrutural vivo (COMP_*/EVT_*/ESC_*/EVT_SUBPROC_*/TIMER_*) |
| `ReviewThread` / `ReviewStore` | threads/pins ancorados + ciclo EM REVISÃO (`requestChanges`) | governança de revisão (editor ↔ studio) |
...
| `SimulationState` / `Token` / `Decision` / `DecisionOutcome` | estado de tokens (+ cartões `errorThrowOptions`/`escalationThrowOptions`/`compensateCard`/`eventSubprocessOptions`) | simulação |
...
| `Messages` (i18n) | mapas chave→string (EN/PT_BR), montados por composição de 18 fragmentos `fragments/*.ts` | textos de UI |
...
do seu escopo (≈305 seções de arquivo no total).
