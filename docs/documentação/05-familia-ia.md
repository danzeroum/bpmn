...
> Δ 2026-07: contratos EXPANDIDOS por H16-19. `SimNode` ganhou `eventKind`/`eventRef`/`eventRefLabel` (E-6, matching de eventos), `eventSubprocess?` + `esubStart?: EsubStartInfo` (ES-5, casca de event subprocess elegível). Novos tipos: `EsubStartInfo` (startId/kind/ref?/refLabel?/interrupting), `ErrorThrowOption` (E-6), `EscalationDestination`+`EscalationThrowOption` (H18 §5e, destino predito sem disparar), `CompensationDestination`+`CompensateCard`+`CompensationStep`+`CompensationPlan` (H19 §6d/§6e), `EventSubprocessOption` (ES-5 §4e, disparo manual timer/conditional). `Decision` ganhou 6 variantes: `error{host,errorRef?}`, `escalation{host,escalationRef?}`, `signal{ref}`, `message{ref}`, `eventSubprocess{sub,atStep}`, `compensate{scope?,activityRef?,waitForCompletion,atStep}`. `BoundaryOption` ganhou `eventKind?`/`eventRef?`/`catchAll?`. `TransitionRecord.type` ganhou `'event'`. `SimulationState` ganhou `errorThrowOptions`/`escalationThrowOptions`/`compensateCard`/`eventSubprocessOptions`.
**Papel:** Tipos de valor públicos do engine headless de simulação de token (todos JSON serializáveis/determinísticos).
**Entradas:** Nenhuma.
**Processamento (intermediário):** Nenhum.
**Saídas:** Tipos.
**Estruturas de dados que trafegam:** `GatewayKind` (exclusive/parallel/inclusive/eventBased), `SimEdge` (id/source/target/label), `SimNode` (id/type/label/gateway?/outgoing/incoming/boundaryHost?/interrupting?/**eventKind?/eventRef?/eventRefLabel?/eventSubprocess?/esubStart?**/isStart/isEnd), `EsubStartInfo` (startId/kind/ref?/refLabel?/interrupting — o start tipado único da casca elegível, derivado por `startIsInterrupting`), `PendingChoice`, `BoundaryOption` (host/boundary/interrupting/label/eventKind?/eventRef?/catchAll?), `ErrorThrowOption` (host/hostLabel/options[{errorRef?,label?}] — a IU escolhe o ERRO, o engine resolve o destino por matching), `EscalationDestination` (união boundary/esubStart{label,interrupting} | dissolve | ambiguous{candidates}), `EscalationThrowOption` (host/hostLabel/options[{escalationRef?,label?,destination}] — destino predito SEM disparar, reforço 7), `CompensationDestination` (broadcast{handlerCount,esubLabels} | activity{handlerLabel} | notEligible{reason}), `CompensateCard` (scopeLabel/options — 1 card scope-wide), `CompensationStep` (activityId/activity/handlerId?/handler?/reason?), `CompensationPlan` (steps/compensated/uncompensated/esubLabels/blocked? — plano READ-ONLY consumido por `compensate()` E pela cola de ledger), `EventSubprocessOption` (sub/subLabel/startId/kind/interrupting), `Decision` (união: exclusive/eventBased{gateway,edge} | inclusive{gateway,edges} | boundary{host,boundary} | decision{node,context} | **error{host,errorRef?}** | **escalation{host,escalationRef?}** | **signal{ref}** | **message{ref}** | **eventSubprocess{sub,atStep}** | **compensate{scope?,activityRef?,waitForCompletion,atStep}**), `DecisionOutcome` (outputs?/ruleIndex?/noMatch?/nonSimulable{cell,reason}), `DecisionEvaluator` (hasDecision/inputsOf/evaluate — a **interface injetada** do caminho runtime), `PendingDecisionInput`, `BlockedDecision` (nodeId/cell/reason), `TransitionRecord` (step/type{move/split/join-wait/join-fire/boundary/**event**/decision/decision-blocked/end}/message/nodeId?/edgeId?/approximate?), `Token` (id/nodeId), `SimulationState` (tokens/joinArrivals/traversedEdges/visitedNodes/trail/complete/deadlocked/pendingChoice/boundaryOptions/**errorThrowOptions/escalationThrowOptions/compensateCard/eventSubprocessOptions**/pendingDecisionInput/blockedDecision), `SimulationOptions` (scope?/decisions?).
...
> Δ 2026-07: cada `SimNode` agora recebe os campos de MATCHING de eventos (H16 E-6): `eventKind` (`properties.eventDefinition`), `eventRef` (`eventDefinitionRefOf`) e `eventRefLabel` (nome resolvido por `findEventDefinition` para message/signal/error/escalation, cai no ref). E os campos de EVENT SUBPROCESS (H17 ES-5): `eventSubprocess` (via `isEventSubprocess`) e `esubStart` computado por `eligibleStartOf` — só presente quando a casca é ELEGÍVEL (exatamente um `startEvent` tipado entre os filhos diretos; casca degenerada = `undefined`, correção é do lint 4d). `startIsInterrupting` (fonte única ES-1) decide o modo — nunca predicado local.
**Papel:** Constrói o grafo de controle de fluxo (SimGraph) para um escopo, com a mesma classificação de nó/aresta que a análise de soundness; agora também anexa os metadados de evento/event-subprocess que o matching (E-6/ES-5) e os cards de throw consomem.
**Entradas:** `BpmnDiagram`, `scope?`.
**Processamento (intermediário):** `inScope: Set<string>` filtra flow nodes do escopo; monta `SimNode` por nó (gateway via `gatewayKindOf`, boundary via `boundaryAttachedTo`/`isNonInterrupting`); anexa `eventKind`/`eventRef`/`eventRefLabel` quando `properties.eventDefinition` é string; `eligibleStartOf(diagram, subId)` (helper local) lê `childrenOf` filtrando `startEvent` ativos — se houver exatamente 1 com kind string, monta `EsubStartInfo` (kind, `interrupting` via `startIsInterrupting`, `ref` via `eventDefinitionRefOf`, `refLabel` via `findEventDefinition`); popula `outgoing`/`incoming` percorrendo `activeEdges` (ignora tipos em `NON_FLOW_EDGE_TYPES`: messageFlow/association/dataAssociation); `label` de aresta cai no label do target; sets/maps transitórios `nodes`/`edges`/`starts`/`boundariesByHost`/`inScope`.
...
**Estruturas de dados que trafegam:** `SimGraph`, `SimNode`, `SimEdge`, `EsubStartInfo`, `GatewayKind`, `BpmnDiagram`, `BpmnNode`, `EventDefinitionRefKind`.

### `packages/simulation/src/engine.ts`
> Δ 2026-07: `SimulationError` estende `BpmnError` (code `SIMULATION`).
> Δ 2026-07: eventos lançados com MATCHING honesto (H16 E-6): `throwError`/`throwSignal`/`throwMessage` — a IU escolhe o evento (por definição nomeada ou o não-catalogado), o ENGINE resolve o destino por matching (nunca chute); `boundaryOptions` deixa de listar boundaries de erro (viram card de throw); `errorThrowOptions` são os cards por host.
> Δ 2026-07: disparo de EVENT SUBPROCESS com precedência + interrupção nomeada (H17 ES-5): `throwError`/`throwSignal`/`throwMessage` passam a incluir as event-subprocess-starts elegíveis do escopo como candidatas; ordem TOTAL declarada (especificidade > escopo > catch-all: 1 esub exato, 2 boundary exato, 3 esub catch-all, 4 boundary catch-all). >1 candidato no tier vencedor = `BlockedDecision` nomeando candidatos. `catchByEventSubprocess` coloca token na casca (descida não simulada) e, se interrupting (`startIsInterrupting`), cancela os tokens pré-existentes do escopo exatamente 1×. Timer/conditional NUNCA auto-disparam: `eventSubprocessOptions` + `fireEventSubprocess` são o card manual.
> Δ 2026-07: `throwEscalation` (H18 EC-5): mesma ordem total que `throwError` (construída sobre `eligibleEscalationCatches` do core, nunca re-derivada — via `escalationTiers`), mas o contraste vinculante — SEM catch elegível a escalação DISSOLVE (no-op declarado no trail, o token do host SEGUE, `this.blocked` NUNCA é setado; ao contrário do erro não-capturado, que é STOP). Catch não-interrupting: host segue + token paralelo. `predictEscalation`/`escalationThrowOptions` predizem o destino+modo por opção SEM disparar (reforço 7).
> Δ 2026-07: `compensate()` reverso pós-conclusão (H19 CO-4): compensa as atividades COMPLETADAS do escopo em ordem REVERSA. `completedActivityIds` deriva do trail (`'move'` saindo de um `ACTIVITY_TYPES`; última conclusão vence em loops). `compensationPlan(activityRef?)` é READ-ONLY (nunca muta trail/estado) e é a FONTE ÚNICA que `compensate()` e a cola de ledger consomem. Broadcast (sem `activityRef`) = todo handler `⟲` em ordem reversa + as event-subprocess de compensação do escopo (`compensationEsubs`); alvo específico = só seu handler (esub NÃO participa, reforço 9). Alvo não-compensável/não-completado = STOP declarado (nada executa → ledger não anexa). `compensateCard` é 1 card scope-wide.
**Papel:** Engine de token small-step determinístico; resolve XOR/AND/event/boundary exato e OR aproximado por dominadores; a lista ordenada de `Decision` É o scenario (replay bit-a-bit). Desde H16-19 também resolve eventos lançados (error/escalation/signal/message), disparo de event subprocess e compensação — todos com falha DECLARADA, nunca resultado silencioso.
**Entradas:** `BpmnDiagram` + `SimulationOptions` (com `decisions?: DecisionEvaluator` injetado); `Decision` (choose/replay — roteada por `kind` a fireBoundary/decideDecision/throwError/throwEscalation/throwSignal/throwMessage/fireEventSubprocess/compensate); `boundaryId`; `host`+`errorRef?`/`escalationRef?`; `ref` (signal/message); `subId`; `scope?`+`activityRef?`+`waitForCompletion`; `Scenario` (replay). Importa do core `compensableActivitiesOf`/`eligibleEscalationCatches`/`nodeParentId`/`BpmnError`.
**Processamento (intermediário):** Estado interno mutável: `tokens: Token[]`, `joinArrivals: Map<string,Set>`, `traversedEdges`/`visitedNodes: Set`, `trail: TransitionRecord[]`, `decisions: Decision[]`, `tokenSeq`/`stepSeq`, `blocked: BlockedDecision|null`; `dom` (immediate-dominators). Const local `ACTIVITY_TYPES` (o que conta como atividade compensável — headless, nunca consulta registry). `reset()` semeia start events (ou sources sem incoming, EXCLUINDO cascas de event subprocess — nunca implicit start). Micro-passos: `step`/`emit`/`deliver` (move/split, sync-join com contagem de arrivals, OR-join adiado); `settleOrJoins` (fixpoint), `orJoinReady`/`canReach` (BFS + dominância); **`decideDecision`** chama `decisionSupport.evaluate(node,context)` → `DecisionOutcome`, roteia pelo primeiro output (`String(values[0])` casado com label de aresta), ou faz `stop()` produzindo `BlockedDecision` + registro `decision-blocked` (nonSimulable/noMatch/output sem flow). **Eventos lançados (E-6/ES-5):** `throwError` monta 4 tiers e resolve o primeiro não-vazio (1 candidato→captura via boundary `applyBoundary` ou esub `catchByEventSubprocess`; 0→STOP não-capturado; >1→STOP ambíguo); `throwEscalation` (via `escalationTiers` sobre `eligibleEscalationCatches`) idem mas 0→DISSOLVE (no-op); `throwSignal` faz broadcast a TODO catch em espera (`waitingCatches`) + toda esub casada (`matchingEventSubprocesses`), 0=no-op; `throwMessage` entrega a 1 destino único, 0=no-op, >1=STOP (correlação runtime não simulável). **Compensação (§6d):** `compensationPlan` monta steps reversos (broadcast) ou 1 (alvo); `compensate` captura `preExisting` ANTES de colocar tokens, coloca handlers em ordem reversa (registro `event` numerado), dispara `compensationEsubs`, registra `waitForCompletion`. `compensationHandlerOf` resolve o handler pela association que sai do boundary `⟲` (associations vivem no diagrama, não no flow graph). `isActivity`/`scopeLabel` auxiliares. Locais transitórios abundantes: `values`, `summary`, `wanted`, `edgeId`, `arrivals`, `live`, `seen`, `queue`, `tiers`, `winner`, `preExisting`, `victims`, `steps`, `completed`.
**Saídas:** `StepResult {moved, transitions}`; getters `state: SimulationState`, `transitions`, `pendingChoice`, `boundaryOptions`, `errorThrowOptions`, `escalationThrowOptions`, `compensateCard`, `eventSubprocessOptions`, `pendingDecisionInput`, `scenario: Scenario`; `compensationPlan()` (read-only, para a cola de ledger); `SimulationEngine` (replay estático — âncora `atStep` para eventSubprocess/compensate); erros `SimulationError`.
**Estruturas de dados que trafegam:** `SimulationState`, `Token`, `TransitionRecord`, `Decision`, `DecisionOutcome`, `DecisionEvaluator`, `PendingChoice`, `PendingDecisionInput`, `BlockedDecision`, `BoundaryOption`, `ErrorThrowOption`, `EscalationThrowOption`, `EscalationDestination`, `CompensateCard`, `CompensationDestination`, `CompensationPlan`, `CompensationStep`, `EventSubprocessOption`, `SimNode`, `Scenario` (diagramId/versionId/semanticVersion/scope/decisions), `StepResult`, `SimGraph`.
...
> Δ 2026-07: `canonicalDecision` ganhou os shapes estáveis das novas variantes de `Decision` (H16-19): `error{host,errorRef?}`, `escalation{host,escalationRef?}`, `signal`/`message{ref}`, `eventSubprocess{sub,atStep}`, `compensate{scope?,activityRef?,waitForCompletion,atStep}` — o `atStep` ancora QUANDO o disparo ocorreu (a interrupção/o conjunto revertido dependem dele), então faz parte do roteiro replayável.
**Papel:** JSON canônico e hash de um Scenario (roteiro) para evidência SACM/ledger.
**Entradas:** `Scenario` (canonicalizeScenario/hashScenario).
**Processamento (intermediário):** `canonicalDecision` normaliza cada `Decision` via `switch` por `kind` (ordena `edges` do inclusive, ordena chaves de `context` do decision, emite `errorRef`/`escalationRef`/`scope`/`activityRef` só quando presentes, preserva `atStep`); `JSON.stringify` com ordem fixa de chaves; `sha256Hex` (do core) truncado em 12 hex.
**Saídas:** `string` (JSON canônico, `canonicalizeScenario`); `Promise<string>` (hash 12 chars, `hashScenario`).
...
> Δ 2026-07: passou a reexportar os novos tipos de H16-19: `CompensateCard`, `CompensationDestination`, `CompensationPlan`, `CompensationStep`, `ErrorThrowOption`, `EscalationDestination`, `EscalationThrowOption`, `EsubStartInfo`, `EventSubprocessOption`.
**Papel:** Barrel público do pacote simulation.
**Saídas:** Re-exporta graph (buildSimGraph/flowScopeOf/gatewayKindOf/isFlowNode/SimGraph), engine (+Scenario/StepResult), coverage, scenario, session e todos os tipos (Decision, DecisionEvaluator, DecisionOutcome, os cards de throw/compensação, etc.).
...
- **`Decision` / `Scenario` / `SimulationSession` / `CoveragePath` / `CoverageSummary`** — o roteiro replayável (lista ordenada de decisões, agora incluindo eventos lançados error/escalation/signal/message, disparo de event subprocess e compensação — estes dois ancorados por `atStep`), seu hash canônico e a evidência de cobertura SACM/ledger; dominadores (`idom`) sustentam a convergência OR-join.
- **`ErrorThrowOption` / `EscalationThrowOption` / `CompensateCard` / `EventSubprocessOption` / `EsubStartInfo`** (simulation, H16-19) — os cards de decisão manual do simulador: lançar erro/escalação por definição nomeada (o engine resolve o destino por matching, ordem total especificidade>escopo>catch-all), compensar em ordem reversa pós-conclusão, disparar event subprocess timer/conditional; escalação sem catch DISSOLVE (no-op declarado), erro sem catch é STOP — o contraste vinculante do H18.
